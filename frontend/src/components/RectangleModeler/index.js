...
const RectangleModeler = ({ onDataChange }) => {
  // Grid configuration
  const GRID_SIZE = 30;
  const GRID_WIDTH = 20;
...
const clearCanvas = () => {
  ...
  // Add this useEffect to expose data and functions to the parent component
  useEffect(() => {
    // Expose the rectangle and equation data to the parent component
    window.rectanglesData = rectangles;
    window.equationsData = equations;
    window.clearCanvasFunction = clearCanvas;
    
    // If you provided onDataChange prop, use it to notify the parent
    if (onDataChange) {
      onDataChange({
        rectangles,
        equations
  ...
  }, [rectangles, equations, onDataChange]);
